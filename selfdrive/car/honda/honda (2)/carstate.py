...
def get_can_signals(CP):
  ...
  signals = [
      ...
      ("STEER_STATUS", "STEER_STATUS", 0), #needs 0 until we fix steer_status
  ...
  if CP.carFingerprint == CAR.CIVIC:
    ...
    signals += [("MAIN_ON", "SCM_BUTTONS", 0),
                ("CAR_GAS", "GAS_PEDAL", 0)]
               # ("LKAS_DISABLED", "STEER_MOTOR_TORQUE",0)]
...
class CarState(CarStateBase):
  def __init__(self, CP):
   ...
   # self.steer_status_values = defaultdict(lambda: "UNKNOWN", can_define.dv["STEER_STATUS"]["STEER_STATUS"]) needs to be fixed for V2 board
  ...
  def update(self, cp, cp_cam, cp_body):
    ...
    if self.CP.carFingerprint in (CAR.CRV, CAR.CRV_EU, CAR.HRV, CAR.ODYSSEY, CAR.ACURA_RDX, CAR.RIDGELINE, CAR.PILOT_2019, CAR.ODYSSEY_CHN, CAR.ACCORD_2016, CAR.ACURA_MDX):
